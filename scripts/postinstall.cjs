...
try {
  ...
  if (termCols < 60) {
    out.write(`\n  ${B}unicode-spinners${R} ${D}— 18 braille spinners${R}\n\n`);
    cleanup();
    return;
  }
  ...
  const timer = setInterval(() => {
    if (Date.now() - start >= DURATION) {
      ...
      let bot = CL + '\n';
      const cmds = [
        ['npx unicode-spinners',        'demo all spinners'],
        ['npx unicode-spinners --list',  'list all spinners'],
        ['npx unicode-spinners --web',   'open in browser'],
      ];
